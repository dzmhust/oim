...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/oimCleanBag/find', idField : 'id',singleSelect:true,
    ...
    columns:[[    
    			...
    			,{field:'name',title:'清洁包名称',sortable:true,width:100,align:'left',halign:'center' }
	    			...
	    			,{field:'description',title:'清洁包描述',sortable:true,width:100,align:'left',halign:'center' }
	    			,{field:'operateRegion',title:'适用区域',sortable:true,width:100,align:'left',halign:'center' }
    		    	    ]],
    sortName:'effectiveDate',sortOrder:'asc',
 ...
 */
function add(){
	dForm=$('#dlg').dialog({title:'新增清洁包',width: 800, height: 600,modal:true
		,href:ctx+'/oimCleanBag/add',
		buttons:[
...
function edit(){
	...
	dForm=$('#dlg').dialog({title:'编辑清洁包',width: 800, height: 600,modal:true
		,href:ctx+'/oimCleanBag/edit/'+row.id,
		buttons:[
			{text:'提交',handler:function(){submit();}}
			,{text:'返回',handler:function(){dForm.panel('close');}}
		]
...
function view(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
	dForm=$('#dlg').dialog({title:'查看清洁包',width: 800, height: 600,modal:true
		,href:ctx+'/oimCleanBag/view/'+row.id,
		buttons:[
			{text:'返回',handler:function(){dForm.panel('close');}}
		]
	});
...
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/oimCleanBag/remove/"+row.id,
...
function setRegion(){
	...
	operateCityDailog=$('#dlg').dialog({title:'选择运营区域',width: 380, height: 500, modal:true
		,href:ctx+'/oimCleanBag/showRegionDialog'
	});
}
/**
 * 关闭运营区域
...
function closeRegion(datas){
	...
	$.ajax({
		type:'post',
		url:ctx+"/oimCleanBag/setRegions",
...
function enable(){
	...
	parent.$.messager.confirm('提示', '您确定要启用吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/oimCleanBag/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	...
	parent.$.messager.confirm('提示', '您确定要禁用？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/oimCleanBag/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function submit(){
	$('#mainform').form('submit',{
		onSubmit:function(){
			...
			if (isValid){
																																																																																														var datasOimCleanBagLine = $('#OimCleanBagLineGrid').datagrid('getData');
										var datasJsonOimCleanBagLine = JSON.stringify(datasOimCleanBagLine.rows)
					$('#datasOimCleanBagLine').val(datasJsonOimCleanBagLine);
								$.messager.progress();
			}
