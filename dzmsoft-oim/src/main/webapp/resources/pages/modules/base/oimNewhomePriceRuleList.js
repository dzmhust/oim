...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/oimNewhomePriceRule/find', idField : 'id',singleSelect:true,
...
function setRegion(){
	...
	operateCityDailog=$('#dlg').dialog({title:'选择运营区域',width: 380, height: 500, modal:true
		,href:ctx+'/oimNewhomePriceRule/showRegionDialog'
	});
}
/**
 * 关闭运营区域
...
function closeRegion(datas){
	...
	$.ajax({
		type:'post',
		url:ctx+"/oimNewhomePriceRule/setRegions",
 ...
 */
function add(){
		var url = ctx+'/oimNewhomePriceRule/add';
...
function edit(){
	var row = dgList.datagrid('getSelected');
	if(rowIsNull(row)) return;
		var url = ctx+'/oimNewhomePriceRule/edit/'+row.id;
...
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/oimNewhomePriceRule/remove/"+row.id,
...
function enable(){
	...
	parent.$.messager.confirm('提示', '您确定要启用吗？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/oimNewhomePriceRule/enable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
...
function disable(){
	...
	parent.$.messager.confirm('提示', '您确定要禁用？', function(data){
		if (data){
			$.ajax({
				type:'put',
				url:ctx+"/oimNewhomePriceRule/disable/"+row.id,
				success: function(data){
					if (data.success){
						$.messager.alert('系统提示',data.msg,'info',function(){
							find();
