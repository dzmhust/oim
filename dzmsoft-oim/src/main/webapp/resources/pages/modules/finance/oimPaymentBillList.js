...
function createDg(){
	dgList=$('#dg').datagrid({    
	method: "post",url:ctx+'/oimPaymentBill/find', idField : 'id',singleSelect:true,
...
function edit(){
	...
	if(rowIsNull(row)) return;
		...
		,href:ctx+'/oimPaymentBill/edit/'+row.id,
		buttons:[
			{text:'提交',handler:function(){submit();}}
			,{text:'返回',handler:function(){dForm.panel('close');}}
		]
...
function remove(){
	...
	parent.$.messager.confirm('提示', '删除后无法恢复您确定要删除？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:ctx+"/oimPaymentBill/remove/"+row.id,
